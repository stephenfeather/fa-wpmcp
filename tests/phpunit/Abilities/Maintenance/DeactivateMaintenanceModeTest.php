...
final class DeactivateMaintenanceModeTest extends TestCase {
	...
	public function test_ability_metadata(): void {
		$ability = new DeactivateMaintenanceMode();
		$this->assertEquals( 'fa-wpmcp/deactivate-maintenance-mode', $ability->getName() );
		$this->assertEquals( 'maintenance', $ability->getCategory() );
		$this->assertEquals( 'Deactivate Maintenance Mode', $ability->getLabel() );
		$this->assertStringContainsString( 'deactivate', strtolower( $ability->getDescription() ) );
